        ...
        <div class="container">
            ...
            <div class="row justify-content-center">
                <div class="col-lg-8">
                    ...
                    <!-- Total Summary if multiple orders -->
                    @if($donHangs->count() > 1)
                        <div class="alert alert-info text-center mb-4">
                            <strong>Đơn hàng của bạn đã được chia thành {{ $donHangs->count() }} đơn nhỏ theo từng cửa hàng.</strong>
                        </div>
                    @endif
                                    ...
                                    <tbody>
                                        @foreach($donHang->chiTiet as $item)
                                            ...
                                            <tr>
                                                ...
                                                <td class="col-price">{{ number_format($item->Gia, 0, ',', '.') }} đ</td>
                                                <td>{{ $item->SoLuong }}</td>
                                        ...
                                        <div class="summary-details">
                                            ...
                                            @if($donHang->giamGia)
                                                <div class="d-flex justify-content-between text-success">
                                                    @if($donHang->giamGia->LoaiGiamGia === 'TienMat')
                                                        <span>Giảm giá (Mã {{ $donHang->giamGia->MaCode }}):</span>
                                                    @else
                                                        <span>Giảm giá ({{ $donHang->giamGia->PhanTram }}%):</span>
                                                    @endif
                                                    <span>
                                                        -{{ number_format(max(0, $donHang->chiTiet->sum('ThanhTien') + $donHang->PhiVanChuyen - $donHang->TongTien), 0, ',', '.') }} đ
                                                    </span>
                                                </div>
                                            @endif
                                            <div class="d-flex justify-content-between">
                                                <span>Phí vận chuyển:</span>
                                                @if($donHang->PhiVanChuyen > 0)
                                                    <span class="text-dark">{{ number_format($donHang->PhiVanChuyen, 0, ',', '.') }} đ</span>
                                                @else
                    ...
                    <div class="card next-steps-card">
                        ...
                        <div class="card-body">
                            <div class="timeline">
                                <div
                                    class="timeline-item {{ $donHangs->first()->TrangThai === 'Chờ xử lý' ? 'active' : 'completed' }}">
                                ...
                                <div class="timeline-item {{ $donHangs->first()->TrangThai === 'Đang giao' ? 'active' : '' }}">
                                    <div class="timeline-marker">
                                ...
                                <div class="timeline-item {{ $donHangs->first()->TrangThai === 'Hoàn thành' ? 'active' : '' }}">
                                    <div class="timeline-marker">
                    ...
                    <div class="text-center mt-4">
                        ...
                        @if(Route::has('orders.history'))
                            <a href="{{ route('orders.history') }}" class="btn btn-info">
