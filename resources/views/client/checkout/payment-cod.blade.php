                    ...
                    <div class="card shadow-sm border-0">
                        <div class="card-body p-4">
                            ...
                            <div class="text-center mb-4">
                                @if($donHangs->first()->PhuongThucThanhToan === 'COD')
                                    <div class="mb-3">
                            ...
                            <div class="text-center mb-4">
                                ...
                                @else
                                    <span class="badge bg-info px-3 py-2" style="font-size: 14px;">
                                        <i class="fas fa-wallet"></i> Thanh toán qua {{ $donHangs->first()->PhuongThucThanhToan }}
                                    </span>
                                @endif
                            </div>
                            ...
                            <div class="border-top border-bottom py-3 mb-4">
                                <div class="row mb-2">
                                    ...
                                    <div class="col-6 text-end fw-bold">{{ $maDonHang }}</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-6 text-muted">Tổng tiền:</div>
                                    <div class="col-6 text-end fw-bold text-primary">{{ number_format($donHangs->sum('TongTien'), 0, ',', '.') }} đ</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-6 text-muted">Phương thức:</div>
                                    <div class="col-6 text-end">{{ $donHangs->first()->PhuongThucThanhToan }}</div>
                                </div>
                                <div class="row mb-2">
                                    <div class="col-6 text-muted">Trạng thái:</div>
                                    <div class="col-6 text-end">
                                        <span class="badge bg-{{ $donHangs->first()->TrangThai === 'Đã xác nhận' ? 'success' : 'warning' }}">
                                            {{ $donHangs->first()->TrangThai }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <p class="mb-1"><strong>Người nhận:</strong> {{ $donHangs->first()->TenKhachHang }}</p>
                                <p class="mb-1"><strong>Số điện thoại:</strong> {{ $donHangs->first()->SoDienThoai }}</p>
                                <p class="mb-1"><strong>Địa chỉ:</strong> {{ $donHangs->first()->DiaChiGiaoHang }}</p>
                                @if($donHangs->first()->GhiChu)
                                    <p class="mb-1"><strong>Ghi chú:</strong> {{ $donHangs->first()->GhiChu }}</p>
                                @endif
                            </div>
                            ...
                            @if($donHangs->first()->PhuongThucThanhToan === 'COD')
                                <div class="alert alert-success mb-4">
                                    ...
                                    <p class="mb-0">Vui lòng chuẩn bị số tiền <strong>{{ number_format($donHangs->sum('TongTien'), 0, ',', '.') }} đ</strong> để thanh toán khi nhận hàng.</p>
                                </div>
                            @else
                                <div class="alert alert-info mb-4">
                                    <h6 class="alert-heading"><i class="fas fa-check-circle me-2"></i>Thanh toán đã được xác nhận</h6>
                                    <p class="mb-2">Giao dịch qua <strong>{{ $donHangs->first()->PhuongThucThanhToan }}</strong> đã được ghi nhận.</p>
