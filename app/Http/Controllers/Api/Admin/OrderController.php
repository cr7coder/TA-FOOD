...
class OrderController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $mappedData = [
            ...
            'CommissionRate' => $firstNhaHang ? (float)($firstNhaHang->commission_rate ?? 15.0) : 15.0,
            'CommissionAmount' => $firstNhaHang ? round($tamTinh * ((float)($firstNhaHang->commission_rate ?? 15.0) / 100), 0) : round($tamTinh * 0.15, 0),
            'CommissionAmountFormat' => number_format($firstNhaHang ? round($tamTinh * ((float)($firstNhaHang->commission_rate ?? 15.0) / 100), 0) : round($tamTinh * 0.15, 0), 0, ',', '.') . ' đ',
            'TotalMerchantOwed' => ($tamTinh - ($firstNhaHang ? round($tamTinh * ((float)($firstNhaHang->commission_rate ?? 15.0) / 100), 0) : round($tamTinh * 0.15, 0))) + ((float)$order->PhiVanChuyen - $carrierCost),
            'TotalMerchantOwedFormat' => number_format(($tamTinh - ($firstNhaHang ? round($tamTinh * ((float)($firstNhaHang->commission_rate ?? 15.0) / 100), 0) : round($tamTinh * 0.15, 0))) + ((float)$order->PhiVanChuyen - $carrierCost), 0, ',', '.') . ' đ',
