...
class GioHangController extends Controller
{
    ...
    private function formatCartResponse(GioHang $gioHang)
    {
        ...
        $items = $gioHang->chiTiet->map(function ($ct) {
            return [
                ...
                'image'    => $ct->monAn ? $ct->monAn->hinh_anh_url : '/images/no-image.png',
    ...
    {
        // AJAX / JSON requests → return cart data (used by floating cart popup)
        if ($request->expectsJson() || $request->ajax() || $request->header('X-Requested-With') === 'XMLHttpRequest') {
            $gioHang = $this->resolveCart();
            return $this->formatCartResponse($gioHang);
        }

        // Browser navigation → render the cart page view
    ...
    public function store(Request $request)
    {
        ...
        if (empty($activeCategoryNames)) {
            $activeCategoryNames = ['Cơm', 'Bún', 'Phở', 'Mì', 'Trà', 'Trà sữa', 'Đồ ăn vặt', 'Món chay'];
        }
    ...
    public function update(Request $request, $maMonAn)
    {
        ...
        if (!$monAn || $monAn->TrangThai !== 'Còn bán' || (!empty($monAn->DanhMuc) && !in_array($monAn->DanhMuc, $activeCategoryNames))) {
            return response()->json([
                'success' => false,
                'message' => 'Món ăn đã hết hàng hoặc không khả dụng (7E.4)'
            ], 422);
        }
